...
library(tidyverse)
library(quantmod)   # Para acceder a los datos
...
series = c('NAEXKP01RUQ657S', 'RUSCPIALLQINMEI') # FRED codes for Russia GDP growth and CPI
Growth = getSymbols(series[1], src='FRED', auto.assign=FALSE)
CPI    = getSymbols(series[2], src='FRED', auto.assign=FALSE)

```

## Calculamos la tasa de inflación
...
Data = inner_join(tibble(Date=time(Growth), Growth=coredata(Growth)), 
  ...
  drop_na() # Quitamos los valores ausentes
...
pivot_longer(Data, cols=-Date, names_to="Variables", values_to="Values") %>% 
  ...
  labs(title="Russia GDP growth and CPI inflation", x="", y="",
       caption=paste0("Source: FRED series ", paste(series, collapse=", ")))

```

## Creamos modelo para estimar VAR
...
## Graficos de predicción temporal
...
ggplot(forecast_data) + 
  ...
  labs(title="Forecasts of Russia GDP growth and CPI inflation", 
